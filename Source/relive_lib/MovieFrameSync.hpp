...
// (see Movie.cpp), tests wire it to a scripted fake so the drop/wait/render decisions can be
// verified without real audio/video/threads.
...
enum class MovieFrameOutcome
{
    ...
    Wait,               // frame is ahead of the audio clock - keep it and ask again next tick
...
// timestamp (mkv PTS in ms). Never blocks: a frame that's ahead of the audio clock returns
